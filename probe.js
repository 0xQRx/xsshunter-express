...
function check_for_vue_js() {
    const originalWarn = console.warn;
    let isVueDetected = false;
  
    // Override console.warn to intercept the Vue.js warning
    console.warn = function(message, ...args) {
      if (message.includes('Vue is a constructor and should be called with the "new" keyword')) {
        isVueDetected = true;
      }
      originalWarn.call(console, message, ...args); // Call the original warn function
    };
  
    // Try calling Vue as a function to trigger the warning
    try {
      Vue();
    } catch (e) {
      // Handle errors if Vue() throws
    }
  
    // Restore the original console.warn
    console.warn = originalWarn;
  
    // Check for elements with Vue-specific classes or IDs
    const vueClassOrId = Array.from(document.querySelectorAll('*')).filter(el => {
      const id = el.id || '';
      const className = typeof el.className === 'string' ? el.className : '';
      return id.includes('vue') || className.includes('vue');
    });
  
    if (vueClassOrId.length > 0) {
      isVueDetected = true;
    }
  
    // Return the detection result
    return isVueDetected;
  }
...
function hook_load_if_not_ready() {
    try {
        ...
        // Check for Vue.js and decide on canvas creation
        if (check_for_vue_js()) {
            //console.log("Vue.js detected; skipping html2canvas.");
            probe_return_data['screenshot'] = ''; // No screenshot for Vue.js pages
            finishing_moves(); // Proceed without the canvas
        } else {
            // Create canvas and capture screenshot only if Vue.js is not detected
            if (typeof html2canvas === "function") {
                html2canvas(document.body).then(function (canvas) {
                    probe_return_data['screenshot'] = canvas.toDataURL();
                    finishing_moves();
                }).catch(function (e) {
                    //console.error("Error during html2canvas execution:", e);
                    probe_return_data['screenshot'] = ''; // Fallback to no screenshot
                    finishing_moves();
                });
            } else {
        ...
        //console.error("Error in hook_load_if_not_ready:", e);
        probe_return_data['screenshot'] = ''; // Fallback to no screenshot
        finishing_moves();
    }
}
